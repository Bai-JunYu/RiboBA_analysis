...
SIM_DIR="${PUBLIC_INPUTS_DIR}/data/sim_compare"
...
if [[ ! -d "${PUBLIC_INPUTS_DIR}" ]]; then
  ...
  echo "Usage: bash run_tools_ncorf_benchmark.sh [PUBLIC_INPUTS_ROOT]" >&2
  exit 1
fi
...
CANDIDATE_ORF="${PUBLIC_INPUTS_DIR}/result/prepare/riborf_atg/candidateORF.genepred.txt"
...
for fastq in ./*.fastq; do
  ...
  bowtie -p 14 -v 1 -a -m 10 --best --strata -S --no-unal \
